...
class SurveyWithoutDiary extends Component {
    constructor(props) {
        super(props);
        this.state = {
            recipes: [],
            ...
            for (let n=0; n<dishTypesStages.length; n++) {
                var recipesReceived = []
          	    snapshot.forEach(function(child) {
                    if (child.val().attributes.course.includes(dishTypesStages[n]) && child.val().dietType.includes('withoutDiary')) {  
...
export default SurveyWithoutDiary